...
export async function generateMetadata({ params, searchParams }) {
  ...
  const title = maker && model
    ? `${maker} ${model} Stock (${totalCount.toLocaleString() ?? 0} units) | REAL MOTOR JAPAN`
    : `REAL MOTOR JAPAN - Car Stock`;

  const description = maker && model
    ? `Browse our ${maker} ${model} stock. Used Japanese Cars`
    : "Used Japanese Cars";
...
//   const { maker, model } = searchParams || {};

//   const title =
//     maker && model
//       ? `${maker} ${model} Stock | ${companyData.name}`
//       : `${companyData.name} - Car Stock`;

//   const description =
//     maker && model
//       ? `Browse our ${maker} ${model} stock. ${companyData.summary}`
//       : companyData.summary;
...
// Define the page component accepting searchParams
const CarStock = async ({ params, searchParams }) => {
  ...
  const carFilters = {
    ...
    model: decodeURIComponent(model?.toUpperCase()) || null,
  }
  let claims = null
